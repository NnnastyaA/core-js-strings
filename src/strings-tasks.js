...
function repeatString(str, times) {
  const emptyLine = '';
  if (times > 0) {
    const RepeatString = str.repeat(times);
    return RepeatString;
  }
  return emptyLine;
}
...
function removeFirstOccurrences(str, value) {
  const indexLine = str.indexOf(value);

  if (indexLine === -1) {
    return str;
  }

  const newLine = str.slice(0, indexLine) + str.slice(indexLine + value.length);
  return newLine;
}
...
function removeLastOccurrences(str, value) {
  const lastIndexLine = str.lastIndexOf(value);

  if (lastIndexLine === -1) {
    return str;
  }
  const newLine =
    str.slice(0, lastIndexLine) + str.slice(lastIndexLine + value.length);
...
function sumOfCodes(str) {
  ...
  for (let i = 0; i < str.length; i += 1) {
    const lettersCode = str.charCodeAt(i);
    sumLetter += lettersCode;
  }
  return sumLetter;
}
...
function orderAlphabetically(str) {
  ...
  const newLine = arrLetters.sort().join('');
  return newLine;
}
...
function countVowels(str) {
  const countNumber = str.match(/[aeiouy]/gi);
  return countNumber === null ? 0 : countNumber.length;
}
...
function unbracketTag(str) {
  const stringWithoutBrackets = str.substring(1, str.length - 1);
...
function encodeToRot13(str) {
  ...
  for (let i = 0; i < str.length; i += 1) {
    ...
    const indexLetter = normalAlphabet.indexOf(currentLetter);

    if (indexLetter === -1) {
      newLine += currentLetter;
    } else {
      const encodedLetter = encAlphabet.charAt(indexLetter);
      newLine += encodedLetter;
    }
  }
